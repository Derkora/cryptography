...
def xor_shift_bin(bin_plaintext, bin_key):
    ...
    for i in range(0, len(bin_plaintext), block_size):
        bin_pt_block = bin_plaintext[i:i + block_size]
        bin_key_block = bin_key[i % len(bin_key):i % len(bin_key) + block_size]  

        xor_result = ''.join(format(int(b1) ^ int(b2), '01b') for b1, b2 in zip(bin_pt_block, bin_key_block))
        shifted = xor_result[1:] + xor_result[0]  
        result += shifted
...
def encrypt_ecb(bin_plaintext, bin_key):
    return xor_shift_bin(bin_plaintext, bin_key)

# Encryption function for CBC
def encrypt_cbc(bin_plaintext, bin_key, bin_iv):
    block_size = 4
    previous_block = bin_iv  
    result = ""
    
    for i in range(0, len(bin_plaintext), block_size):
        bin_pt_block = bin_plaintext[i:i + block_size]
        
        xor_result = xor_bin(bin_pt_block, previous_block)

        shifted_block = xor_shift_bin(xor_result, bin_key)

        result += shifted_block
        
        previous_block = shifted_block

    return result

# Streamlit UI
st.title("Encrypt XOR ECB and CBC With 1 Bit Shift")
